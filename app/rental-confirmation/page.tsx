...
function RentalConfirmationPageContent() {
  ...
  useEffect(() => {
      ...
      if (activeRentals.length > 1) {
        ...
        const updatedRentals = rentals.map((rental: any) => {
          if (
            ...
            rental.id !== sortedActiveRentals[0].id
          ) {
            return {
              ...rental,
              status: "completed",
    ...
    const userRentals = cleanedRentals.filter(
      (rental: any) => rental.userId === user.id,
    );
    ...
    if (!hasActiveRental) {
      ...
      console.log(
        "No active rental found, but showing latest rental:",
        latestRental,
      );
    ...
    if (latestRental.status !== "active") {
      const activeRental = userRentals.find(
        (rental: any) => rental.status === "active",
      );
      if (activeRental) {
          ...
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
            <div className="bg-gradient-to-r from-orange-500 to-orange-600 px-4 sm:px-6 lg:px-8 py-8 sm:py-12 text-center">
              ...
              <p className="text-orange-100 text-base sm:text-lg">
                We found your rental but it's not currently active
              </p>
            </div>
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-2 sm:ml-3">
                    <p className="text-xs sm:text-sm text-yellow-700">
                      ...
                      {latestRental.status.charAt(0).toUpperCase() +
                        latestRental.status.slice(1)}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-start">
                ...
                <div className="ml-2 sm:ml-3">
                  <p className="text-xs sm:text-sm text-orange-700">
                    <span className="font-medium">Active Rental:</span> You can
                    end your ride anytime using the "End My Ride" button.
                    {getStatusText(latestRental.status, latestRental).includes(
                      "Overdue",
                    ) && (
                      <span className="block mt-1 font-semibold text-xs sm:text-sm">
                        ⚠️ Your rental is overdue - please end your ride soon!
                      </span>
                    )}
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4">
                ...
                <div className="space-y-3 sm:space-y-4">
                  ...
                  <div className="flex items-center justify-between p-3 sm:p-4 bg-gray-50 rounded-lg">
                    ...
                    <span
                      className={`px-2 py-1 text-xs rounded-full font-medium border ${getStatusColor(latestRental.status, latestRental)}`}
                    >
            ...
            <div className="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-gray-200">
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                ...
                >
                  ...
                  {(latestRental?.status as string) === "completed"
                    ? "Ride Completed"
                    : "Return Bike"}
                </button>
              </div>

...
function ManageSlots() {
  ...
  const [newStatus, setNewStatus] = useState<
    "active" | "in-maintenance" | "occupied" | "reserved"
  >("active");
  ...
  const fetchSlots = () => {
    try {
      ...
      if (
        ...
        existingSlots.length !== targetSlotCount
      ) {
      ...
      const activeRentals = rentals.filter(
        (rental: any) =>
          rental.status === "active" && rental.station === stationName,
      );
      ...
      const updatedSlots = existingSlots.map((slot: Slot) => {
        const activeRental = activeRentals.find(
          (rental: any) => rental.slotNumber === slot.slotNumber,
        );
  ...
  const openStatusChangeModal = (
    slot: Slot,
    newStatus: "active" | "in-maintenance" | "occupied" | "reserved",
  ) => {
  ...
  const confirmStatusChange = () => {
    ...
    try {
      ...
      alert(
        `Slot ${selectedSlot.slotNumber} status updated to ${newStatus === "active" ? "Active" : "In-Maintenance"}`,
      );

      // Trigger a custom event to notify parent page of changes
      window.dispatchEvent(
        new CustomEvent("slotStatusChanged", {
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl p-4 text-white shadow-xl">
            <div className="text-center">
              <div className="text-2xl font-bold mb-1">
                {
                  slots.filter((slot) => slot.status === "in-maintenance")
                    .length
                }
              </div>
                ...
                {slots.map((slot) => {
                  ...
                  return (
                    <tr key={slot.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`px-3 py-1 text-xs rounded-full font-semibold flex items-center w-fit ${getStatusColor(slot.status)}`}
                        >
                      ...
                      <td className="px-6 py-4 text-sm text-gray-900 max-w-xs">
                        <div
                          className={`truncate ${slot.notes?.includes("Reserved for bike returns") ? "font-semibold text-purple-600" : ""}`}
                        >
                          {slot.notes || "-"}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex space-x-2">
                          {slot.status === "reserved" ? (
                            ...
                            <button
                              onClick={() =>
                                openStatusChangeModal(slot, "in-maintenance")
                              }
                            ...
                            <button
                              onClick={() =>
                                openStatusChangeModal(slot, "active")
                              }
        ...
        {slots.length === 0 && (
          <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8 text-center">
            ...
            <p className="text-gray-600">
              No slots available for this station.
            </p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-600">
                Change slot {selectedSlot.slotNumber} status
              </p>
            </div>

        ...
        try {
          ...
          videoRef.current.onloadedmetadata = async () => {
            try {
              ...
              console.error('Video play failed after metadata:', secondPlayError);
  ...
  useEffect(() => {
    if (isScanning) {
      const interval = setInterval(() => {
        ...
        if (randomChance < 0.1) { // 10% chance to "detect" a QR code
          ...
          router.push(`/bike-selection?station=${mockQRData.split('-')[2]}&bike=${mockQRData.split('-')[1]}`);
        }
      }, 1000);
             ...
             <div className="bg-gray-100 rounded-xl p-4 mb-6 relative overflow-hidden">
               ...
               <video
                 ...
                 className={`w-full h-48 object-cover rounded-lg bg-black ${
                   isScanning && !isCameraLoading && !cameraError ? 'block' : 'hidden'
                 }`}
                 onError={(e) => {
